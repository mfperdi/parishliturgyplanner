...
function ASSIGNMENT_buildVolunteerMap(volunteerData) {
  ...
  for (const row of volunteerData) {
    ...
    const massPrefs = (row[cols.PREFERRED_MASS_TIME - 1] || "").split(',').map(s => s.trim()); // Column 11 (Mass times)
    const familyTeam = row[cols.FAMILY_TEAM - 1] || null; // Column 8
    
    // ROLE PREFERENCES: Column 12 - Ministry role preferences
    const rolePrefs = (row[cols.MINISTRY_ROLE_PREFERENCE - 1] || "").split(',').map(s => s.trim().toLowerCase());
    
    Logger.log(`DEBUG: Added active volunteer ${name} - Family: ${familyTeam}, MassPrefs: [${massPrefs.join(', ')}], RolePrefs: [${rolePrefs.join(', ')}]`);
