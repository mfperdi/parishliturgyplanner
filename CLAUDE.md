...
| `2_schedulelogic.gs` | Mass schedule generation with 3-layer logic, backup/restore | `SCHEDULE_generateScheduleForMonth()`, `SCHEDULE_backupAssignments()`, `SCHEDULE_restoreBackup()` |
...
**Timeoffs** - Timeoff request tracking
...
**_AssignmentBackups** - Hidden backup sheet (system-managed)
- Stores automatic backups before destructive operations
- Columns: Timestamp, Month-Year, Backup Data (JSON)
- Keeps last 5 backups per month automatically
- Used by `SCHEDULE_restoreBackup()` for data recovery
...
### Workflow 6: Data Protection & Recovery

**Automatic Backup** (`SCHEDULE_backupAssignments()`):
- Triggered automatically before `SCHEDULE_generateScheduleForMonth()`
- Creates hidden `_AssignmentBackups` sheet if not exists
- Stores JSON snapshot of current month's assignments
- Includes timestamp and month identifier
- Keeps only last 5 backups (auto-cleanup)
- Never throws errors (logs warnings only)

**Manual Recovery** (`SCHEDULE_restoreBackup(monthString)`):
- Call from Apps Script editor: `SCHEDULE_restoreBackup("2025-01")`
- Finds most recent backup for specified month
- Clears current assignments for that month
- Restores all assignments from backup
- Returns success/error message

**Data Protection Strategy**:
1. Automatic backups before destructive operations
2. Hidden sheet prevents accidental user deletion
3. JSON storage preserves exact data state
4. 5-backup limit prevents storage bloat
5. Month-specific recovery allows targeted restoration

**Recovery Scenarios**:
- Accidentally regenerated schedule for wrong month
- Need to revert auto-assignment results
- System error during schedule generation
- Testing/experimentation rollback
