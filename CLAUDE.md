...
- Columns: Date, Time, Description, Liturgical Celebration, Ministry, Role, Event ID, Is Anticipated, Month-Year, Assigned Group, Assigned Volunteer ID, Assigned Volunteer Name, Status
- **Ministry** (Column E): General ministry category (e.g., "Lector", "Eucharistic Minister")
- **Role** (Column F): Specific role within ministry (e.g., "1st reading", "2nd reading", "Bread")
- Generated by Step 2 (Schedule Generation)
- Populated by Step 3 (Auto-Assignment)
- Status: Unassigned, Assigned, Substitute Needed
- **Helper Columns N-P**: Pure Google Sheets formulas for manual assignment support (setup via Admin Tools → Setup Assignment Helper Formulas)
  - **Column N (Qualified?)**: Checks if volunteer has required ministry/role (✓/✗/⚠️)
  - **Column O (Active?)**: Checks if volunteer status is "Active" (✓/⚠️)
  - **Column P (Free?)**: Checks for approved timeoff conflicts (✓/⚠️ CHECK TIMEOFFS)
  - Formulas auto-update when volunteer name entered in column L
  - Not written by schedule/assignment scripts - must be set up separately
  - See Workflow 9 for complete documentation
...
1. User selects optional ministry filter from dropdown (or leaves as "All Ministries")
2. Build liturgical data map for the month
3. Get all assignments for the month
4. **Apply ministry filter** if specified (filters assignments to selected ministry)
5. Group assignments by liturgical celebration (only celebrations with filtered assignments appear)
6. Create formatted sheet with:
   - Parish header
   - Liturgical celebration sections (color-coded)
   - Rank/Season/Color information
   - Mass details grouped by date/time
   - **Ministry and Role columns** (full schedule) or **Role column only** (single-ministry filter)
   - Assigned volunteers
   - Optional summary statistics
7. Apply formatting and column widths (adapts to 5 or 6 columns based on filter)
8. Output to MonthlyView or custom sheet name

**Filtering Behavior**:
- **All Ministries**: Shows all assignments with both Ministry and Role columns (6 columns)
- **Single Ministry** (e.g., "Lector"): Shows only that ministry's assignments with Role column only (5 columns)
- Liturgical celebrations with no matching assignments are excluded from output
- Sheet name can be customized (e.g., "Lector Schedule", "EM Schedule")
...
### Workflow 9: Assignment Helper Formulas

**Trigger**: One-time setup via Admin Tools menu

**Purpose**: Adds real-time eligibility checking formulas to Assignments sheet for manual volunteer assignments

**Process** (`HELPER_FORMULAS_setup()` in 0e_helper_formulas.gs):
1. User runs Admin Tools → Setup Assignment Helper Formulas
2. System adds header row labels to columns N, O, P
3. Creates three pure Google Sheets formulas:
   - **Column N (Qualified?)**: Checks ministry/role qualifications
   - **Column O (Active?)**: Checks volunteer status
   - **Column P (Free?)**: Checks for timeoff conflicts
4. Applies formulas to all assignment rows (row 2 to last row)
5. Formats columns (center alignment, header styling, auto-resize)

**Helper Column Formulas**:

**Column N: Qualified?** ✓/✗
- Checks if volunteer (column L) has required ministry (column E) OR role (column F)
- Looks up volunteer in Volunteers sheet by Full Name (column D)
- Searches volunteer's MINISTRIES (column J) for ministry match
- Searches volunteer's ROLES (column K) for role match
- Returns:
...
- Formulas are **live** - they recalculate automatically when you edit assignments
- Type volunteer name in column L → all three columns update instantly
- No script execution needed after initial setup
- Works even when Apps Script is disabled or quota exceeded

**Use Cases**:
- **Manual assignment workflow**: See eligibility before assigning volunteers
- **Fixing auto-assignment gaps**: Find qualified volunteers for unassigned roles
- **Emergency substitutions**: Quickly verify substitute eligibility
- **Quality control**: Spot-check assignments before publishing schedule
- **Training new coordinators**: Visual feedback helps learn volunteer qualifications

**Setup Requirements**:
1. Assignments sheet must exist (generate schedule first)
2. Volunteers sheet must have data in columns D (Full Name), I (Status), J (Ministries), K (Roles)
3. Timeoffs sheet must have data in columns B (Volunteer Name), D (Selected Dates), G (Status)
4. Ministries sheet should be properly configured for role validation

**Best Practices**:
1. **Run setup once after generating schedule**: Formulas persist across schedule regenerations
...
- **Workflow 3 (Auto-assignment)**: Automated assignment using same eligibility criteria
- **0_diagnostic.gs**: Assignment readiness diagnostic for troubleshooting

**Access Points**:
- Menu: Admin Tools → Setup Assignment Helper Formulas
- Test function: `TEST_helperFormulas()` in Script Editor

**Performance**:
- Formulas recalculate only when relevant cells change
- Minimal performance impact (pure formula calculations)
- Works on large assignment sheets (1000+ rows)

**Troubleshooting**:
- **Formulas not appearing?** Check if Assignments sheet exists and has data
- **⚠️ NOT FOUND errors?** Verify volunteer name exactly matches Volunteers sheet (column D)
- **Column P always shows ✓?** Check Timeoffs sheet has approved timeoffs with dates in correct format
- **Formulas got deleted?** Re-run setup via Admin Tools menu
- **Want to customize formulas?** Edit `0e_helper_formulas.gs` and re-run setup
...
- Prefixes indicate module:
  ...
  - `HELPER_FORMULAS_*()` - Assignment helper formulas
...
**Recent Changes**:
- **Assignment Helper Formulas** (0e_helper_formulas.gs, 0_code.gs):
  ...
  - Complements onEdit validation system (0d_onedit.gs) for comprehensive assignment validation
  - Function: `HELPER_FORMULAS_setup()` adds formulas to all assignment rows
  - Test function: `TEST_helperFormulas()` for verifying formula generation
...
- **Ministry-Based Schedule Filtering** (0a_constants.gs, 2_schedulelogic.gs, 3_assignmentlogic.gs, 5_printschedule.gs, 0_code.gs, Sidebar.html):
  ...
  - Print schedules can be filtered by ministry (single or all) via sidebar dropdown
  - Dynamic ministry dropdown populated from active ministries in Ministries sheet
  - Print output adapts: shows Ministry + Role for full schedule, just Role for single-ministry filter
  - **IMPORTANT**: Existing schedules must be regenerated to populate new Ministry column
  - New server function `getActiveMinistries()` for sidebar ministry list
